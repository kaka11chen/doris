...
public class TemporaryTableMgr extends MasterDaemon {
    ...
    protected void runAfterCatalogReady() {
        Env.getCurrentEnv().clearExpiredSession();
        Map<String, Long> sessionReportTimeMap = Env.getCurrentEnv().getSessionReportTimeMap();
        long currentTs = System.currentTimeMillis();
        InternalCatalog internalCatalog = Env.getCurrentEnv().getInternalCatalog();
        Collection<DatabaseIf<? extends TableIf>> internalDBs = internalCatalog.getAllDbs();
        for (DatabaseIf<? extends TableIf> db : internalDBs) {
            for (TableIf table : db.getTables()) {
                ...
                if (needDelete) {
                    ...
                    try {
                        internalCatalog.dropTableWithoutCheck((Database) db, (Table) table, false, true);
                    } catch (Exception e) {
