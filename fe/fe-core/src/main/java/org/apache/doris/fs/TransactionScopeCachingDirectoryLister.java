...
// import io.trino.filesystem.FileSystem;
        ...
        implements DirectoryLister
{
    ...
    private static final Logger LOG = LogManager.getLogger(TransactionScopeCachingDirectoryLister.class);
    ...
    // private RemoteIterator<RemoteFile> listInternal(FileSystem fs, TableIf table, TransactionDirectoryListingCacheKey cacheKey)
    //         throws IOException
    // {
            ...
            throws IOException
    {
        ...
        FetchingValueHolder existingValue = cache.getIfPresent(cacheKey);
        if (existingValue != null) {
            LOG.info("Cache hit for directory listing: {}", cacheKey.getPath());
            cachedValueHolder = existingValue;
        }
        else {
            try {
                LOG.info("Cache miss for directory listing: {}", cacheKey.getPath());
                cachedValueHolder = cache.get(cacheKey,
                        () -> new FetchingValueHolder(createListingRemoteIterator(fs, table, cacheKey)));
            }
            catch (ExecutionException | UncheckedExecutionException e) {
                Throwable throwable = e.getCause();
                throwIfInstanceOf(throwable, IOException.class);
    ...
    private RemoteIterator<RemoteFile> createListingRemoteIterator(FileSystem fs, TableIf table, TransactionDirectoryListingCacheKey cacheKey)
            throws IOException
    {
    ...
    private RemoteIterator<RemoteFile> cachingRemoteIterator(FetchingValueHolder cachedValueHolder, TransactionDirectoryListingCacheKey cacheKey)
    {
        return new RemoteIterator<RemoteFile>()
        {
                    ...
                    throws IOException
            {
                ...
                checkState(hasNext());
                return cachedValueHolder.getCachedFile(fileIndex++).orElseThrow(() -> new NoSuchElementException());
            }
        };
    }
    ...
    static class FetchingValueHolder
    {
        ...
        private final List<RemoteFile> cachedFiles = synchronizedList(new ArrayList<>());
        @GuardedBy("this")
        @Nullable
