...
TEST_F(VIcebergDeleteSinkTest, TestBuildPositionDeleteBlock) {
    ...
    auto file_path_column = check_and_get_column<ColumnString>(output_block.get_by_position(0).column.get());
    ...
    // Verify pos column
    auto pos_column = check_and_get_column<ColumnInt64>(output_block.get_by_position(1).column.get());
...
TEST_F(VIcebergDeleteSinkTest, TestUnsupportedDeleteType) {
    ...
    VExprContextSPtrs output_exprs;
    auto sink = std::make_shared<VIcebergDeleteSink>(t_eq_delete_sink, output_exprs, nullptr, nullptr);
