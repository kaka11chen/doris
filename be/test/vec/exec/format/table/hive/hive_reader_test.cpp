...
protected:
    ...
    void create_complex_struct_types(DataTypePtr& coordinates_struct_type,
                                      DataTypePtr& address_struct_type, DataTypePtr& phone_struct_type,
                                      DataTypePtr& contact_struct_type,
                                      DataTypePtr& hobby_element_struct_type,
                                      DataTypePtr& hobbies_array_type, DataTypePtr& profile_struct_type,
                ...
                if (table_column_names[i] == "profile") {
                    {
                        ...
                        path1.__set_path({"profile", "address", "coordinates", "lat"});
                        paths.push_back(path1);
                        // address.coordinates.lng
                        TColumnNameAccessPath path2;
                        path2.__set_path({"profile", "address", "coordinates", "lng"});
                        paths.push_back(path2);
                        // contact.email
                        TColumnNameAccessPath path3;
                        path3.__set_path({"profile", "contact", "email"});
                        paths.push_back(path3);
                        // hobbies[].element.level
                        TColumnNameAccessPath path4;
                        path4.__set_path({"profile", "hobbies", "*", "level"});
