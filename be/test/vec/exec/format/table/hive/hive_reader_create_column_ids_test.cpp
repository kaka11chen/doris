...
TEST_F(HiveReaderCreateColumnIdsTest, test_create_column_ids_1) {
    ...
    run_orc_test_with_method({access_config}, expected_column_ids, expected_filter_column_ids, true);
}
...
TEST_F(HiveReaderCreateColumnIdsTest, test_create_column_ids_5) {
    ...
    run_parquet_test_with_method(access_configs, expected_column_ids, expected_filter_column_ids, true);

    run_orc_test_with_method(access_configs, expected_column_ids, expected_filter_column_ids);
    run_orc_test_with_method(access_configs, expected_column_ids, expected_filter_column_ids, true);
}
...
TEST_F(HiveReaderCreateColumnIdsTest, test_create_column_ids_6) {
...

    {
        ...
        std::set<uint64_t> parquet_expected_column_ids = {
                2,  36, 37, 38, 39, 40, 44, 45, 48, 49, 52, 53, 54, 61, 62, 63, 64,
                65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 79, 80, 82, 83};
        // Expected IDs based on the schema: profile(3), address(4), coordinates(7), lat(8), contact(10), email(11)
        std::set<uint64_t> parquet_expected_filter_column_ids = {36, 37, 38, 39, 40};

        run_parquet_test_with_method(access_configs, parquet_expected_column_ids,
...

    {
        // column_ids should contain all necessary column IDs (set automatically deduplicates)
        // Expected IDs based on the schema: name(2), profile(3), address(4), coordinates(7), lat(8), lng(9), contact(10), email(11), hobbies(15), element(16), level(18)
        std::set<uint64_t> orc_expected_column_ids = {2,  36, 37, 38, 39, 40, 44, 45, 48, 49, 52,
                                                      53, 54, 61, 63, 64, 65, 66, 67, 68, 69, 70,
                                                      71, 72, 73, 74, 75, 76, 77, 79, 80, 82, 83};
        // Expected IDs based on the schema: profile(3), address(4), coordinates(7), lat(8), contact(10), email(11)
        std::set<uint64_t> orc_expected_filter_column_ids = {36, 37, 38, 39, 40};
