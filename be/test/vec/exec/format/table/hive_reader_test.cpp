...
class HiveParquetReaderTest : public testing::Test {
public:
    HiveParquetReaderTest() {}
};

TEST_F(HiveParquetReaderTest, BasicReadTest) {
    ...
    ObjectPool obj_pool;
    ASSERT_TRUE(DescriptorTbl::create(&obj_pool, t_desc_table, &desc_tbl));
    ...
    ASSERT_TRUE(p_reader->set_fill_columns(partition_columns, missing_columns));

    auto hive_reader = std::make_unique<HiveParquetReader>(std::move(p_reader), nullptr, nullptr, scan_params, scan_range, nullptr);
    ASSERT_TRUE(hive_reader->init_reader(column_names, nullptr));
    ASSERT_TRUE(hive_reader->init_row_filters());
    ...
    bool eof = false;
    size_t read_row = 0;
    ASSERT_TRUE(hive_reader->get_next_block(block.get(), &read_row, &eof));
