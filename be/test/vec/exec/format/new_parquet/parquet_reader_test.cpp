...
#include "vec/exec/format/new_parquet/vparquet_reader.h"
...
public:
    NewParquetReaderTest() : cache(1024) {}
...
TEST_F(NewParquetReaderTest, normal) {
    TDescriptorTable t_desc_table;
    TTableDescriptor t_table_desc;
    std::vector<std::string> table_column_names = {"tinyint_col", "smallint_col", "int_col",
                                                   "bigint_col",  "boolean_col",  "float_col",
                                                   "double_col"};
    std::vector<TPrimitiveType::type> table_column_types = {
            TPrimitiveType::TINYINT, TPrimitiveType::SMALLINT, TPrimitiveType::INT,
            TPrimitiveType::BIGINT,  TPrimitiveType::BOOLEAN,  TPrimitiveType::FLOAT,
    ...
    auto p_reader = new new_parquet::ParquetReader(nullptr, scan_params, scan_range, 992, &ctz,
                                                   nullptr, nullptr, &cache);
    p_reader->set_file_reader(reader);

    ASSERT_TRUE(p_reader->init().ok());
    RuntimeState runtime_state((TQueryGlobals()));
    runtime_state.set_desc_tbl(desc_tbl);

    std::unordered_map<std::string, ColumnValueRangeType> colname_to_value_range;
    ASSERT_TRUE(p_reader->init_reader(column_names, column_names,
                                      TableSchemaChangeHelper::ConstNode::get_instance(),
                                      converter::HiveColumnTypeConverterFactory::instance().get())
                        .ok());
