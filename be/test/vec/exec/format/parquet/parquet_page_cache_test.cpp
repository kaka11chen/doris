...
TEST(ParquetPageCacheTest, CacheHitReturnsDecompressedPayload) {
    ...
    // ExecEnv::GetInstance()->set_storage_page_cache(StoragePageCache::create_global_cache(1 << 20, 10, 0));
    ...
    //     // Ensure any outstanding PageCacheHandle is released before destroying cache
    //     handle = PageCacheHandle();
    //     ExecEnv::GetInstance()->set_storage_page_cache(nullptr);
    //     delete _cache_ptr;
    //     // Clear CacheManager registration so tests do not leave global state
...
TEST(ParquetPageCacheTest, V2LevelsPreservedInCache) {
    ...
    ColumnChunkReader<false, false> ccr2(&reader, &cc, &field_schema2, nullptr, 0, nullptr);
