...
//            std::hash<std::thread::id> {}(std::this_thread::get_id()));
    ...
    try {
        ...
        // 创建任务句柄
        TaskId task_id("test_task");
        auto task_handle = std::static_pointer_cast<TimeSharingTaskHandle>(TEST_TRY(executor.create_task(
    ...
    try {
        ...
        for (int i = 0;
            ...
            auto task_handle1 = TEST_TRY(executor.create_task(
                    task_id1, []() { return 0.0; }, 10, std::chrono::milliseconds(1), std::nullopt));
            auto task_handle2 = TEST_TRY(executor.create_task(
                    task_id2, []() { return 0.0; }, 10, std::chrono::milliseconds(1), std::nullopt));
            auto task_handle3 = TEST_TRY(executor.create_task(
                    task_id3, []() { return 0.0; }, 10, std::chrono::milliseconds(1), std::nullopt));
