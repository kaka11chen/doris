...
public:
    ...
    void to_filter_conjuncts(std::vector<vectorized::VExprContext* >& filter_conjuncts,
                             const SlotDescriptor *slot_desc,
                             const RuntimeState* state) {
        if (is_fixed_value_range() || is_match_value_range()) {
            // 1. convert to in filter condition
            if (_fixed_values.size() == 0) {
                to_eq_filter_conjunct(filter_conjuncts, slot_desc, state, true);
            } else if (_fixed_values.size() > 1) {
...
//            }
    ...
    void to_eq_filter_conjunct(std::vector<vectorized::VExprContext *>& filter_conjuncts,
                               const SlotDescriptor *slot_desc,
                               RuntimeState* state,
                               bool is_in = true) {
        vectorized::VExpr* root;
        {
            ...
            root = state->obj_pool()->add(new vectorized::VectorizedFnCall(texpr_node));
        }
        {
            //            TExprNode texpr_node;
            ...
            vectorized::VExpr* slot_ref_expr = new vectorized::VSlotRef(slot_desc);
            root->add_child(slot_ref_expr);
        }

        {
            auto* literal_expr = create_literal(state, _fixed_values[0], _precision, _scale);
    ...
    void to_in_filter_conjunct(std::vector<vectorized::VExprContext *>& filter_conjuncts, const RuntimeState* state, bool is_in = true) {
//        vectorized::VExpr* root;
//        {
//            TFunction fn;
//            TFunctionName fn_name;
...
//        }
    ...
    template <class T>
    vectorized::VExpr* create_literal(RuntimeState* state, T value, int precision, int scale) {
        if constexpr (primitive_type == TYPE_DECIMAL32) {
            TExprNode texpr_node;
            ...
            vectorized::VExpr* literal_expr = state->obj_pool()->add(new vectorized::VLiteral(texpr_node));
            return literal_expr;
        } else if constexpr (primitive_type == TYPE_DECIMAL64) {
        } else if constexpr (primitive_type == TYPE_DECIMAL128I) {
        } else if constexpr (primitive_type == TYPE_TINYINT) {
    ...
    void to_match_conjuct(std::vector<TCondition>& filters) {
        for (const auto& value : _match_values) {
            TCondition condition;
            condition.__set_column_name(_column_name);
