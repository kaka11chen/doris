...
Status TimeSharingTaskExecutor::init() {
    ...
    builder.set_min_threads(_runner_threads )
            .set_max_threads(_runner_threads * 2);
...
TimeSharingTaskExecutor::~TimeSharingTaskExecutor() {
    ...
    std::vector<std::shared_ptr<PrioritizedSplitRunner>> splits_to_destroy;
    {
        ...
        for (auto& [task_id, handle] : _tasks) {
            ...
            splits_to_destroy.insert(
                    splits_to_destroy.end(), std::make_move_iterator(task_splits.begin()),
...
Status TimeSharingTaskExecutor::add_task(const TaskId& task_id, std::shared_ptr<TaskHandle> task_handle) {
    std::lock_guard<std::mutex> lock(_mutex);
    _tasks[task_id] = std::dynamic_pointer_cast<doris::vectorized::TimeSharingTaskHandle>(task_handle);
    return Status::OK();
}
...
Result<std::vector<SharedListenableFuture<Void>>> TimeSharingTaskExecutor::enqueue_splits(
    ...
    auto handle = std::dynamic_pointer_cast<TimeSharingTaskHandle>(task_handle);
    {
        ...
        for (const auto& task_split : splits) {
            ...
            auto prioritized_split = PrioritizedSplitRunner::create_shared(
                    handle, split_id, task_split, _ticker);
...
void TimeSharingTaskExecutor::_split_finished(std::shared_ptr<PrioritizedSplitRunner> split, const Status& status) {
