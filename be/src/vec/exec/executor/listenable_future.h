...
public:
    ...
    void set_value(const T& value) {
        _execute(value, doris::Status::OK());
    }

    void set_error(const doris::Status& status) {
        _execute({}, status);
    }
    ...
    SharedListenableFuture<T> share() && {
        return SharedListenableFuture<T>(std::move(*this));
    }
...
public:
    ...
    void add_callback(Callback cb) {
        return impl_->add_callback(std::move(cb));
    }
...
inline SharedListenableFuture<Void> null_future = SharedListenableFuture<Void>::create_ready(Void{});
} // namespace listenable_future
