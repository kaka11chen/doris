...
ColumnIdResult IcebergParquetReader::_create_column_ids(const FieldDescriptor* field_desc,
    ...
    // Check if the Parquet file has valid Iceberg field IDs
    bool has_valid_field_id = false;
    for (int i = 0; i < field_desc->size(); ++i) {
        auto field_schema = field_desc->get_column(i);
        if (field_schema && field_schema->field_id >= 0) {
            has_valid_field_id = true;
            break;
        }
    }

    // Create a mapping from iceberg field_id to FieldSchema for quick lookup (if field IDs exist)
    // Or from lower-case column name to FieldSchema (for early iceberg versions without field IDs)
    std::unordered_map<int, const FieldSchema*> iceberg_id_to_field_schema_map;
    std::unordered_map<std::string, const FieldSchema*> name_to_field_schema_map;

    for (int i = 0; i < field_desc->size(); ++i) {
        auto field_schema = field_desc->get_column(i);
        if (!field_schema) continue;

        if (has_valid_field_id) {
            int iceberg_id = field_schema->field_id;
            iceberg_id_to_field_schema_map[iceberg_id] = field_schema;
        } else {
            // For early Iceberg versions without field IDs, use column name matching
            std::string lower_name = field_schema->name;
            std::transform(lower_name.begin(), lower_name.end(), lower_name.begin(), ::tolower);
            name_to_field_schema_map[lower_name] = field_schema;
        }
    }
    ...
    auto process_access_paths = [](const FieldSchema* parquet_field,
        ...
        if (has_top_level_only) {
            ...
            IcebergParquetNestedColumnUtils::extract_nested_column_ids(*parquet_field, paths,
                                                                       out_ids);
        }
    };
    ...
    for (const auto* slot : tuple_descriptor->slots()) {
        ...
        // Find the corresponding FieldSchema based on whether we have field IDs or not
        const FieldSchema* field_schema = nullptr;
        if (has_valid_field_id) {
            // Use field ID mapping (modern Iceberg)
            ...
            // Use column name mapping (early Iceberg versions)
            std::string lower_col_name = slot->col_name();
            std::transform(lower_col_name.begin(), lower_col_name.end(), 
                          lower_col_name.begin(), ::tolower);
            auto it = name_to_field_schema_map.find(lower_col_name);
            if (it != name_to_field_schema_map.end()) {
                field_schema = it->second;
            }
        }
        ...
        // primitive (non-nested) types: direct mapping
        ...
        // complex types: process nested column access paths
...
ColumnIdResult IcebergOrcReader::_create_column_ids(const orc::Type* orc_type,
    ...
    auto process_access_paths = [](const orc::Type* orc_field,
        ...
        if (has_top_level_only) {
            ...
            IcebergOrcNestedColumnUtils::extract_nested_column_ids(*orc_field, paths, out_ids);
        }
    };
    ...
    for (const auto* slot : tuple_descriptor->slots()) {
        ...
        // nested types: find top-level ORC type first
        auto it = iceberg_id_to_orc_type_map.find(slot->col_unique_id());
        if (it == iceberg_id_to_orc_type_map.end()) {
            continue;
        }
