...
public:
    ...
    Schema(std::vector<NestedField> columns) : Schema(DEFAULT_SCHEMA_ID, std::move(columns)) {}
...
//int main() {
//    // Example usage
//    std::vector<NestedField> columns = {NestedField(1, "name", Type::STRING),
//                                        NestedField(2, "age", Type::INTEGER)};
//    std::unordered_map<std::string, int> aliases = {{"full_name", 1}, {"years", 2}};
//    Schema schema(columns, aliases);
//    std::cout << "Schema ID: " << schema.getSchemaId() << std::endl;
