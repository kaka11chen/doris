...
bool RowGroupReader::_merge_filter(IColumn::Filter &to_filter, std::vector<FilterContext> &from_filters) {
    //struct timespec startT, endT;
    //clock_gettime(CLOCK_MONOTONIC, &startT);
    ...
    for (auto& filter_ctx : from_filters) {
		...
		//fprintf(stderr, "not reverse all one\n");
                continue;
            } else {
		//fprintf(stderr, "not reverse\n");
		...
		//fprintf(stderr, "reverse all zero\n");
                return false;
            } else if (count == 0) { // all one
		//fprintf(stderr, "reverse all one\n");
                //continue;
                /*const size_t size = filter->size();
                for (size_t i = 0; i < size; ++i) {
                    to_filter_data[i] &= (!filter_data[i]);
...
Status RowGroupReader::_filter_block_internal(Block* block,
    ...
    size_t count = filter_size -
                   simd::count_zero_num((int8_t*)filter.data(), filter_size);
    if (count == 0) {
        for (auto& col : columns_to_filter) {
            std::move(*block->get_by_position(col).column).assume_mutable()->clear();
        }
