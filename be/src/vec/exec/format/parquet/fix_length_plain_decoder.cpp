...
Status FixLengthPlainDecoder::_decode_numeric(MutableColumnPtr& doris_column,
    ...
    while (size_t run_length = select_vector.get_next_run<has_filter>(&read_type)) {
        ...
        case ColumnSelectVector::NULL_DATA: {
            ...
            return Status::Error<INTERNAL_ERROR>("error NULL_DATA.");
            //break;
        }
        case ColumnSelectVector::FILTERED_CONTENT: {
            _offset += _type_length * run_length;
            return Status::Error<INTERNAL_ERROR>("error FILTERED_CONTENT.");
            //break;
        }
        case ColumnSelectVector::FILTERED_NULL: {
            // do nothing
            return Status::Error<INTERNAL_ERROR>("error FILTERED_NULL.");
            //break;
        }
        }
    }
