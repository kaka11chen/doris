...
class ParquetReader2 : public GenericReader {
    ENABLE_FACTORY_CREATOR(ParquetReader2);
...
public:
    ...
    ParquetReader2(RuntimeProfile* profile, const TFileScanRangeParams& params,
    ...
    ParquetReader2(const TFileScanRangeParams& params, const TFileRangeDesc& range,
                  io::IOContext* io_ctx, RuntimeState* state);

    ~ParquetReader2() override;
    // for test
...
private:
    ...
    class ParquetLazyColumnLoader : public LazyColumnLoader {
    public:
        ParquetLazyColumnLoader(ParquetReader2* reader, const std::string& col_name, const DataTypePtr& type)
                : _reader(reader), _col_name(col_name), _column_with_type_and_name(type, col_name) {}

        Result<ColumnPtr> load() override {
            SCOPED_RAW_TIMER(&_reader->_statistics.column_read_time);
            bool column_read_eof = false;
            size_t column_read_rows = 0;
            FilterMap filter_map;
            RETURN_IF_ERROR_RESULT(_reader->_current_group_reader->read_column_data(
                    &_column_with_type_and_name, _reader->_batch_size, &column_read_rows, &column_read_eof, filter_map));
//            if (column_read_eof) {
//                return unexpected(Status::EndOfFile("EOF"));
//            }
            return _column_with_type_and_name.column;
        }

    private:
        ParquetReader2* _reader;
        std::string _col_name;
        ColumnWithTypeAndName _column_with_type_and_name;
    };
    ...
    std::unique_ptr<RowGroupReader2> _current_group_reader;
...
//    RowGroupReader2::LazyReadContext _lazy_read_ctx;

    std::list<RowGroupReader2::RowGroupIndex> _read_row_groups;
