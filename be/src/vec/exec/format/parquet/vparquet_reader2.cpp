...
ParquetReader2::ParquetReader2(RuntimeProfile* profile, const TFileScanRangeParams& params,
...
ParquetReader2::ParquetReader2(const TFileScanRangeParams& params, const TFileRangeDesc& range,
...
ParquetReader2::~ParquetReader2() {
    _close_internal();
}

void ParquetReader2::_init_profile() {
    if (_profile != nullptr) {
        static const char* parquet_profile = "ParquetReader2";
...
Status ParquetReader2::close() {
    _close_internal();
    return Status::OK();
}

void ParquetReader2::_close_internal() {
    if (!_closed) {
        _closed = true;
    }
}

Status ParquetReader2::_open_file() {
...
const FieldDescriptor ParquetReader2::get_file_metadata_schema() {
    return _file_metadata->schema();
}

Status ParquetReader2::open() {
    RETURN_IF_ERROR(_open_file());
    _t_metadata = &(_file_metadata->to_thrift());
    return Status::OK();
}

void ParquetReader2::_init_system_properties() {
...
void ParquetReader2::_init_file_description() {
...
//void ParquetReader2::iceberg_sanitize(const std::vector<std::string>& read_columns) {
//    if (_file_metadata != nullptr) {
//        _file_metadata->iceberg_sanitize(read_columns);
//    }
//}

Status ParquetReader2::init_reader(
...
//    _slot_id_to_filter_conjuncts = slot_id_to_filter_conjuncts;
    ...
    _read_columns = std::move(file_column_names);
...
//    _lazy_read_ctx.conjuncts = conjuncts;
    if (!_read_columns.empty()) {
        std::cout << "[Parquet Reader] Columns to read(" << _read_columns.size() << "):\n";
        for (size_t i = 0; i < _read_columns.size(); ++i) {
            std::cout << "  " << i+1 << ". " << _read_columns[i] << "\n";
        }
    } else {
        std::cout << "[Parquet Reader] No columns selected for reading\n";
    }
    RETURN_IF_ERROR(_init_row_groups(filter_groups));
    return Status::OK();
}

Status ParquetReader2::set_fill_columns(
...
Status ParquetReader2::get_parsed_schema(std::vector<std::string>* col_names,
...
Status ParquetReader2::get_columns(std::unordered_map<std::string, TypeDescriptor>* name_to_type,
...
Status ParquetReader2::get_next_block(Block* block, size_t* read_rows, bool* eof) {
    fprintf(stdout, "ParquetReader2::get_next_block\n");
...
//    }
    ...
    // Create a LazyColumnLoader for each column
    Block lazy_block;
    for (const auto& col_name : _read_columns) {
        auto loader = std::make_shared<ParquetLazyColumnLoader>(this, col_name, block->get_by_name(col_name).type);
        auto lazy_column = LazyColumn::create(std::move(loader));
        // Add the LazyColumn to the block
        lazy_block.insert(ColumnWithTypeAndName(std::move(lazy_column), std::make_shared<DataTypeLazy>(block->get_by_name(col_name).type), col_name));
    }
    block->swap(std::move(lazy_block));

    // TODO: Design another interface don't use these vars.
    *eof = false;
    *read_rows = _batch_size;
    return Status::OK();
...
//RowGroupReader2::PositionDeleteContext ParquetReader2::_get_position_delete_ctx(
//        const tparquet::RowGroup& row_group, const RowGroupReader2::RowGroupIndex& row_group_index) {
//    if (_delete_rows == nullptr) {
//        return RowGroupReader2::PositionDeleteContext(row_group.num_rows, row_group_index.first_row);
//    }
...
//    return RowGroupReader2::PositionDeleteContext(*_delete_rows, row_group.num_rows,
//                                                 row_group_index.first_row, start_index, end_index);
//}

Status ParquetReader2::_next_row_group_reader() {
    ...
    if (_read_row_groups.empty()) {
        _row_group_eof = true;
        _current_group_reader.reset(nullptr);
        return Status::EndOfFile("No next RowGroupReader2");
    }
    RowGroupReader2::RowGroupIndex row_group_index = _read_row_groups.front();
...
//    RowGroupReader2::PositionDeleteContext position_delete_ctx =
//            _get_position_delete_ctx(row_group, row_group_index);
    ...
    _current_group_reader.reset(new RowGroupReader2(
...
Status ParquetReader2::_init_row_groups(const bool& is_filter_groups) {
...
std::vector<io::PrefetchRange> ParquetReader2::_generate_random_access_ranges(
        const RowGroupReader2::RowGroupIndex& group, size_t* avg_io_size) {
    ...
    std::function<void(const FieldSchema*, const tparquet::RowGroup&)> scalar_range =
            [&](const FieldSchema* field, const tparquet::RowGroup& row_group) {
                if (field->type.type == TYPE_ARRAY) {
                    ...
                    fprintf(stdout, "field->physical_column_index: %d\n", field->physical_column_index);
...
bool ParquetReader2::_is_misaligned_range_group(const tparquet::RowGroup& row_group) {
...
bool ParquetReader2::_has_page_index(const std::vector<tparquet::ColumnChunk>& columns,
                                    PageIndex& page_index) {
    return page_index.check_and_get_page_index_ranges(columns);
}

Status ParquetReader2::_process_page_index(const tparquet::RowGroup& row_group,
                                          std::vector<RowRange>& candidate_row_ranges) {
    if (UNLIKELY(_io_ctx && _io_ctx->should_stop)) {
        return Status::EndOfFile("stop");
    }
...
Status ParquetReader2::_process_row_group_filter(const tparquet::RowGroup& row_group,
                                                bool* filter_group) {
...
Status ParquetReader2::_process_column_stat_filter(const std::vector<tparquet::ColumnChunk>& columns,
                                                  bool* filter_group) {
    if ((!_enable_filter_by_min_max) || _colname_to_value_range == nullptr ||
        _colname_to_value_range->empty()) {
        return Status::OK();
...
void ParquetReader2::_init_chunk_dicts() {}

Status ParquetReader2::_process_dict_filter(bool* filter_group) {
    return Status::OK();
}

void ParquetReader2::_init_bloom_filter() {}

Status ParquetReader2::_process_bloom_filter(bool* filter_group) {
    return Status::OK();
}

int64_t ParquetReader2::_get_column_start_offset(const tparquet::ColumnMetaData& column) {
    return has_dict_page(column) ? column.dictionary_page_offset : column.data_page_offset;
}

void ParquetReader2::_collect_profile() {
    if (_profile == nullptr) {
        return;
    }
...
void ParquetReader2::_collect_profile_before_close() {
    _collect_profile();
}

SortOrder ParquetReader2::_determine_sort_order(const tparquet::SchemaElement& parquet_schema) {
