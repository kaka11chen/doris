...
#include "vec/exec/format/new_parquet/parquet_pred_cmp.h"
#include "vec/exec/format/new_parquet/vparquet_column_reader.h"
...
public:
    ...
    Status init_reader(const std::vector<std::string>& table_column_names,
    ...
    const tparquet::FileMetaData* meta_data() const { return _t_metadata; }
...
private:
    ...
    bool _has_page_index(const std::vector<tparquet::ColumnChunk>& columns,
                         new_parquet::PageIndex& page_index);
...
private:
    ...
    std::vector<std::string> _file_column_names;
