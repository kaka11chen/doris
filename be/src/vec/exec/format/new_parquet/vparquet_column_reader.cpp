...
#include "vec/exec/format/new_parquet/level_decoder.h"
...
Status ScalarColumnReader::read_column_data(
        ...
        converter::ColumnTypeConverterFactory* convert_factory) {
    if (_converter == nullptr) {
        _converter = PhysicalToLogicalConverter::get_converter(_field_schema,
                                                               _field_schema->data_type, type, _ctz,
                                                               convert_factory, is_dict_filter);
...
Status ArrayColumnReader::read_column_data(
    ...
    RETURN_IF_ERROR(_element_reader->read_column_data(
            ...
            read_rows, eof, is_dict_filter, convert_factory));
    if (*read_rows == 0) {
        return Status::OK();
    }
...
Status MapColumnReader::read_column_data(
    ...
    RETURN_IF_ERROR(_key_reader->read_column_data(key_column, key_type, root_node->get_key_node(),
                                                  ...
                                                  is_dict_filter, convert_factory));
    ...
    while (value_rows < key_rows && !value_eof) {
        ...
        RETURN_IF_ERROR(_value_reader->read_column_data(
                ...
                key_rows - value_rows, &loop_rows, &value_eof, is_dict_filter, convert_factory));
    ...
    for (size_t i = 0; i < doris_struct.tuple_size(); ++i) {
        ...
        if (not_missing_column_id == -1) {
            ...
            RETURN_IF_ERROR(_child_readers[file_name]->read_column_data(
                    ...
                    batch_size, &field_rows, &field_eof, is_dict_filter, convert_factory));
            ...
            *eof = field_eof;
            ...
            while (field_rows < *read_rows && !field_eof) {
                ...
                RETURN_IF_ERROR(_child_readers[file_name]->read_column_data(
                        ...
                        filter_map, *read_rows - field_rows, &loop_rows, &field_eof, is_dict_filter,
                        convert_factory));
