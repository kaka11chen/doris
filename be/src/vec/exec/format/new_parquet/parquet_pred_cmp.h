...
public:
    ...
    static Status get_min_max_value(const FieldSchema* col_schema, const std::string& encoded_min,
        ...
        auto converter = PhysicalToLogicalConverter::get_converter(
                col_schema, logical_data_type, logical_data_type, &ctz, convert_factory);
    ...
    // // Backward compatibility method that creates a default factory
    // static Status get_min_max_value(const FieldSchema* col_schema, const std::string& encoded_min,
    //                                 const std::string& encoded_max, const cctz::time_zone& ctz,
    //                                 Field* min_field, Field* max_field) {
    //     converter::HiveColumnTypeConverterFactory default_factory;
    //     return get_min_max_value(col_schema, encoded_min, encoded_max, ctz, &default_factory,
    //                            min_field, max_field);
    // }
    ...
    static bool check_can_filter(OP op, const std::vector<Field>& literal_values,
        ...
        if (!ParquetPredicate::get_min_max_value(col_schema, column_stat.encoded_min_value,
                                                 column_stat.encoded_max_value, *ctz,
                                                 convert_factory, &min_field, &max_field)) {
            return false;
        };
    ...
    //                              cctz::time_zone* ctz) {
    //     converter::HiveColumnTypeConverterFactory default_factory;
    //     return check_can_filter(op, literal_values, column_stat, col_schema, ctz, &default_factory);
    // }
};
