...
class InListPredicateBase2 : public ColumnPredicate {
public:
    using T = typename PrimitiveTypeTraits<Type>::CppType;
    template <typename ConditionType, typename ConvertFunc>
    InListPredicateBase2(uint32_t column_id, const ConditionType& conditions,
    ...
    InListPredicateBase2(uint32_t column_id, const std::shared_ptr<HybridSetBase>& hybrid_set,
    ...
    ~InListPredicateBase2() override = default;
    ...
    __attribute__((flatten)) void _evaluate_vec_internal(const vectorized::IColumn& column,
        ...
        if (column.is_nullable()) {
            ...
            _base_loop_vec<false, is_and>(size, flags, nullptr, data_array);
        }
...
private:
    ...
    uint16_t _base_evaluate(const vectorized::IColumn* column,
        ...
        if (column->is_column_dictionary()) {
            ...
            auto& pred_col =
                    vectorized::check_and_get_column<
                            const vectorized::ColumnVector<int>>(column)
                            ->get_data();
...
#undef EVALUATE_WITHOUT_NULL_IMPL
    ...
    template <bool is_nullable, bool is_and, typename TArray>
    __attribute__((flatten)) void _base_loop_vec(uint16_t size, bool* __restrict bflags,
                                                 const uint8_t* __restrict null_map,
                                                 const TArray* __restrict data_array) const {
        //uint8_t helps compiler to generate vectorized code
        uint8_t* flags = reinterpret_cast<uint8_t*>(bflags);
        if constexpr (is_and) {
            for (uint16_t i = 0; i < size; i++) {
                if constexpr (is_nullable) {
                    flags[i] &= (uint8_t)(!null_map[i] && _operator(_values->find(reinterpret_cast<const T*>(&data_array[i])), true));
                } else {
                    flags[i] &= (uint8_t)_operator(_values->find(reinterpret_cast<const T*>(&data_array[i])), true);
                }
            }
        } else {
            for (uint16_t i = 0; i < size; i++) {
                if constexpr (is_nullable) {
                    flags[i] = !null_map[i] && _operator(_values->find(reinterpret_cast<const T*>(&data_array[i])), true);
                } else {
                    flags[i] = _operator(_values->find(reinterpret_cast<const T*>(&data_array[i])), true);
                }
            }
        }
    }
    ...
    std::string _debug_string() const override {
        std::string info =
                "InListPredicateBase2(" + type_to_string(Type) + ", " + type_to_string(PT) + ")";
        return info;
    }
...
template <PrimitiveType Type, PredicateType PT, typename ConditionType, typename ConvertFunc,
          size_t N = 0>
ColumnPredicate* _create_in_list_predicate2(uint32_t column_id, const ConditionType& conditions,
                                           const ConvertFunc& convert, bool is_opposite = false,
    ...
    if constexpr (N >= 1 && N <= FIXED_CONTAINER_MAX_SIZE) {
        using Set = std::conditional_t<
                ...
                HybridSet<Type, FixedContainer<T, N>,
                          const vectorized::ColumnVector<int>>>;
        return new InListPredicateBase2<Type, PT, Set>(column_id, conditions, convert, is_opposite,
                                                      col, arena);
    } else {
        using Set = std::conditional_t<
...
template <PrimitiveType Type, PredicateType PT, typename ConditionType, typename ConvertFunc>
ColumnPredicate* create_in_list_predicate2(uint32_t column_id, const ConditionType& conditions,
                                          const ConvertFunc& convert, bool is_opposite = false,
                                          const TabletColumn* col = nullptr,
                                          vectorized::Arena* arena = nullptr) {
    if (conditions.size() == 1) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 1>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 2) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 2>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 3) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 3>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 4) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 4>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 5) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 5>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 6) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 6>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == 7) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc, 7>(
                column_id, conditions, convert, is_opposite, col, arena);
    } else if (conditions.size() == FIXED_CONTAINER_MAX_SIZE) {
        return _create_in_list_predicate2<Type, PT, ConditionType, ConvertFunc,
...
ColumnPredicate* _create_in_list_predicate2(uint32_t column_id,
    ...
    if constexpr (N >= 1 && N <= FIXED_CONTAINER_MAX_SIZE) {
        using Set = std::conditional_t<
                ...
                HybridSet<Type, FixedContainer<T, N>,
                          const vectorized::ColumnVector<int>>>;
        return new InListPredicateBase2<Type, PT, Set>(column_id, hybrid_set, char_length);
    } else {
        using Set = std::conditional_t<
                std::is_same_v<T, StringRef>, StringSet<DynamicContainer<std::string>>,
                HybridSet<Type, DynamicContainer<T>,
...
ColumnPredicate* create_in_list_predicate2(uint32_t column_id,
                                          const std::shared_ptr<HybridSetBase>& hybrid_set,
                                          size_t char_length = 0) {
    if (hybrid_set->size() == 1) {
        return _create_in_list_predicate2<Type, PT, 1>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 2) {
        return _create_in_list_predicate2<Type, PT, 2>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 3) {
        return _create_in_list_predicate2<Type, PT, 3>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 4) {
        return _create_in_list_predicate2<Type, PT, 4>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 5) {
        return _create_in_list_predicate2<Type, PT, 5>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 6) {
        return _create_in_list_predicate2<Type, PT, 6>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == 7) {
        return _create_in_list_predicate2<Type, PT, 7>(column_id, hybrid_set, char_length);
    } else if (hybrid_set->size() == FIXED_CONTAINER_MAX_SIZE) {
        return _create_in_list_predicate2<Type, PT, FIXED_CONTAINER_MAX_SIZE>(column_id, hybrid_set,
                                                                             char_length);
    } else {
        return _create_in_list_predicate2<Type, PT>(column_id, hybrid_set, char_length);
    }
}
