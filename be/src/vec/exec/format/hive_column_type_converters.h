...
/**
 * Hive-compatible implementation of column type converter factory
 * Follows Apache Hive's type conversion semantics
 */
class HiveColumnTypeConverterFactory : public ColumnTypeConverterFactory {
public:
    static std::shared_ptr<ColumnTypeConverterFactory> instance();

    std::unique_ptr<ColumnTypeConverter> create_converter(const DataTypePtr& src_type,
                                                          const DataTypePtr& dst_type,
                                                          FileFormat file_format) override;

private:
    /**
     * Helper functions for creating different types of converters
     */
    std::unique_ptr<ColumnTypeConverter> _numeric_converter(const DataTypePtr& src_type,
                                                            const DataTypePtr& dst_type);

    template <FileFormat fileFormat = COMMON>
    std::unique_ptr<ColumnTypeConverter> _to_string_converter(const DataTypePtr& src_type,
                                                              const DataTypePtr& dst_type);

    template <FileFormat fileFormat = COMMON>
    std::unique_ptr<ColumnTypeConverter> _from_string_converter(const DataTypePtr& src_type,
                                                                const DataTypePtr& dst_type);

    std::unique_ptr<ColumnTypeConverter> _numeric_to_decimal_converter(const DataTypePtr& src_type,
                                                                       const DataTypePtr& dst_type);

    std::unique_ptr<ColumnTypeConverter> _datetime_to_numeric_converter(
            const DataTypePtr& src_type, const DataTypePtr& dst_type);

    std::unique_ptr<ColumnTypeConverter> _decimal_to_numeric_converter(const DataTypePtr& src_type,
                                                                       const DataTypePtr& dst_type);

    std::unique_ptr<ColumnTypeConverter> _decimal_converter(const DataTypePtr& src_type,
                                                            const DataTypePtr& dst_type);
};

// Template functions and helper types
