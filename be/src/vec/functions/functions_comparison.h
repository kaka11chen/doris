...
private:
    ...
    template <typename T0, typename T1>
    bool execute_num_right_type2(ColumnsWithTypeAndName& columns_with_type_and_name, size_t result,
    ...
    bool execute_num_right_type_test(Block& block, size_t result,
    ...
    bool execute_num_right_type_test2(ColumnsWithTypeAndName& columns_with_type_and_name,
                                      size_t result, const ColumnVector<Int64>* col_left,
                                      const IColumn* col_right_untyped) {
        //        struct timespec startT, endT;
        const ColumnConst* col_right = down_cast<const ColumnConst*>(col_right_untyped);
        //        clock_gettime(CLOCK_MONOTONIC, &startT);
        auto col_res = ColumnUInt8::create();
        //        clock_gettime(CLOCK_MONOTONIC, &endT);
        //        fprintf(stderr, "==> execute_num_right_type_test2 %lu ns\n", (endT.tv_sec - startT.tv_sec) * 1000000000 + (endT.tv_nsec - startT.tv_nsec));
    ...
    template <typename T0, typename T1>
    bool execute_num_const_right_type2(ColumnsWithTypeAndName& columns_with_type_and_name,
                                       size_t result, const ColumnConst* col_left,
                                       const IColumn* col_right_untyped) {
        if (const ColumnVector<T1>* col_right =
                    check_and_get_column<ColumnVector<T1>>(col_right_untyped)) {
            auto col_res = ColumnUInt8::create();
    ...
    template <typename T0>
    bool execute_num_left_type2(ColumnsWithTypeAndName& columns_with_type_and_name, size_t result,
                                const IColumn* col_left_untyped, const IColumn* col_right_untyped) {
        if (const ColumnVector<T0>* col_left =
                    check_and_get_column<ColumnVector<T0>>(col_left_untyped)) {
            if (execute_num_right_type2<T0, UInt8>(columns_with_type_and_name, result, col_left,
                                                   col_right_untyped) ||
                execute_num_right_type2<T0, UInt16>(columns_with_type_and_name, result, col_left,
                                                    col_right_untyped) ||
                execute_num_right_type2<T0, UInt32>(columns_with_type_and_name, result, col_left,
                                                    col_right_untyped) ||
                execute_num_right_type2<T0, UInt64>(columns_with_type_and_name, result, col_left,
                                                    col_right_untyped) ||
                execute_num_right_type2<T0, Int8>(columns_with_type_and_name, result, col_left,
                                                  col_right_untyped) ||
                execute_num_right_type2<T0, Int16>(columns_with_type_and_name, result, col_left,
                                                   col_right_untyped) ||
                execute_num_right_type2<T0, Int32>(columns_with_type_and_name, result, col_left,
                                                   col_right_untyped) ||
                execute_num_right_type2<T0, Int64>(columns_with_type_and_name, result, col_left,
                                                   col_right_untyped) ||
                execute_num_right_type2<T0, Int128>(columns_with_type_and_name, result, col_left,
                                                    col_right_untyped) ||
                execute_num_right_type2<T0, Float32>(columns_with_type_and_name, result, col_left,
                                                     col_right_untyped) ||
                execute_num_right_type2<T0, Float64>(columns_with_type_and_name, result, col_left,
                                                     col_right_untyped))
                return true;
            else {
                LOG(FATAL) << "Illegal column " << col_right_untyped->get_name()
                           ...
                           check_and_get_column_const<ColumnVector<T0>>(col_left_untyped)) {
            if (execute_num_const_right_type2<T0, UInt8>(columns_with_type_and_name, result,
                                                         col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, UInt16>(columns_with_type_and_name, result,
                                                          col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, UInt32>(columns_with_type_and_name, result,
                                                          col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, UInt64>(columns_with_type_and_name, result,
                                                          col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Int8>(columns_with_type_and_name, result,
                                                        col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Int16>(columns_with_type_and_name, result,
                                                         col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Int32>(columns_with_type_and_name, result,
                                                         col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Int64>(columns_with_type_and_name, result,
                                                         col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Int128>(columns_with_type_and_name, result,
                                                          col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Float32>(columns_with_type_and_name, result,
                                                           col_left_const, col_right_untyped) ||
                execute_num_const_right_type2<T0, Float64>(columns_with_type_and_name, result,
                                                           col_left_const, col_right_untyped))
                return true;
            else {
                LOG(FATAL) << "Illegal column " << col_right_untyped->get_name()
    ...
    bool execute_num_left_type_test2(ColumnsWithTypeAndName& columns_with_type_and_name,
                                     size_t result, const IColumn* col_left_untyped,
                                     const IColumn* col_right_untyped) {
        //        ColumnVector<Int64> *col_left = down_cast<ColumnVector<Int64> *>(col_left_untyped);
        ColumnVector<Int64>* col_left = (ColumnVector<Int64>*)(col_left_untyped);
        execute_num_right_type_test2(columns_with_type_and_name, result, col_left,
                                     col_right_untyped);
    ...
    Status execute_decimal2(ColumnsWithTypeAndName& columns_with_type_and_name, size_t result,
    ...
    void execute_generic_identical_types2(ColumnsWithTypeAndName& columns_with_type_and_name,
                                          size_t result, const IColumn* c0, const IColumn* c1) {}
    ...
    Status execute_generic2(ColumnsWithTypeAndName& columns_with_type_and_name, size_t result,
                            const ColumnWithTypeAndName& c0, const ColumnWithTypeAndName& c1) {
