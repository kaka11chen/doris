...
// https://github.com/ClickHouse/ClickHouse/blob/master/src/Common/JoinHashTable/JoinHashTable.h
...
namespace JoinHashTableZeroTraits {
...
/**
  ...
  * LookupResult returned by find() and emplace() methods of JoinHashTable.
  ...
  * types, e.g. in StringJoinHashTable where different components have different
  * Cell format.
  *
  ...
  *    "mapped" value: FixedJoinHashTable or StringJoinHashTable.
  ...
  */
struct JoinHashTableVoidKey {};
struct JoinHashTableVoidMapped {
    template <typename T>
...
/** Compile-time interface for cell of the hash table.
  ...
  */
template <typename Key, typename Hash, typename TState = JoinHashTableNoState>
struct JoinHashTableCell {
    using State = TState;
    ...
    JoinHashTableCell() = default;

    /// Create a cell with the given key / key and value.
    JoinHashTableCell(const Key& key_, const State&) : key(key_) {}
    ...
    static bool is_zero(const Key& key, const State& /*state*/) { return JoinHashTableZeroTraits::check(key); }

    /// Set the key value to zero.
    void set_zero() { JoinHashTableZeroTraits::set(key); }
...
ALWAYS_INLINE inline auto lookup_result_get_key(JoinHashTableCell<Key, Hash, State>* cell) {
    return &cell->key;
}

template <typename Key, typename Hash, typename State>
ALWAYS_INLINE inline void* lookup_result_get_mapped(JoinHashTableCell<Key, Hash, State>*) {
    return nullptr;
}

/**
  * A helper function for JoinHashTable::insert() to set the "mapped" value.
...
struct JoinHashTableZeroValueStorage;

template <typename Cell>
struct JoinHashTableZeroValueStorage<true, Cell> {
private:
    bool has_zero = false;
...
template <typename Cell>
struct JoinHashTableZeroValueStorage<false, Cell> {
    bool get_has_zero() const { return false; }
    void set_get_has_zero() {
        throw doris::Exception(doris::ErrorCode::INVALID_ARGUMENT, "JoinHashTable: logical error");
    }
    void clear_get_has_zero() {}
...
template <typename Key, typename Cell, typename HashMethod,
...
class JoinHashTable : private boost::noncopyable,
                  ...
                  protected JoinHashTableZeroValueStorage<Cell::need_zero_value_storage,
...
protected:
    ...
    uint32_t * buf {nullptr}; /// A piece of memory for all elements except the element with zero key.
    ...
    // PartitionedJoinHashTable will convert this hash table to partitioned hash table
...
//        }
    ...
    std::pair<bool, size_t> ALWAYS_INLINE find_cell_opt(const Key& x, size_t hash_value,
        ...
        bool is_zero = (buf[place_value] == 0);
...
//        }
    ...
    void alloc(const Grower& new_grower, uint32_t _row_num) {
        buf = reinterpret_cast<uint32_t *>(Allocator::alloc(new_grower.buf_size() * sizeof(uint32_t)));
        next = reinterpret_cast<uint32_t *>(Allocator::alloc((row_num + 1) * sizeof(uint32_t)));
        grower = new_grower;
        row_num = _row_num;
    }
    ...
    void free() {
        ...
        if (next) {
            Allocator::free(next, (row_num + 1) * sizeof(uint32_t));
            next = nullptr;
        }
    }
...
//            for (iterator it = begin(), it_end = end(); it != it_end; ++it) it.ptr->~Cell();
    ...
    class iterator_base {
        ...
        using cell_type = std::conditional_t<is_const, const uint32_t, uint32_t>;
    ...
    public:
        iterator_base() {}
        iterator_base(Container* container_, uint32_t* ptr_) : container(container_), ptr(ptr_) {}

        bool operator==(const iterator_base& rhs) const { return ptr == rhs.ptr; }
        bool operator!=(const iterator_base& rhs) const { return ptr != rhs.ptr; }

        Derived& operator++() {
            /// If iterator was pointed to JoinHashTableZeroValueStorage, move it to the beginning of the main buffer.
...
public:
    ...
    void init(uint32_t _row_num) {
        row_num = _row_num;
        alloc(grower, row_num);
    }
    ...
    JoinHashTable& operator=(JoinHashTable&& rhs) {
        ...
        JoinHashTableZeroValueStorage<Cell::need_zero_value_storage, Cell>::operator=(std::move(rhs));
...
//        }
    ...
    void ALWAYS_INLINE emplace_non_zero(KeyHolder&& key_holder, LookupResult& it, bool& inserted,
                                        size_t hash_value, uint32_t row) {
//        const auto& key = key_holder_get_key(key_holder);
...
//        emplace_non_zero_impl(place_value, key_holder, it, inserted, hash_value);
        uint32_t bucket_num = grower.place(hash_value);
//        fprintf(stderr, "emplace_non_zero. bucket_num: %d, hash_value: %ld, row: %d\n", bucket_num, hash_value, row);
        next[row] = buf[bucket_num];
        buf[bucket_num] = row;
    }
...
//    }
    ...
    void ALWAYS_INLINE emplace(KeyHolder&& key_holder, LookupResult& it, bool& inserted,
        ...
        if (!emplace_if_zero(key, it, inserted, hash_value))
            emplace_non_zero(key_holder, it, inserted, hash_value, 0);
    }
    ...
    template <typename KeyHolder>
    void ALWAYS_INLINE emplace2(KeyHolder&& key_holder, LookupResult& it, bool& inserted, uint32_t row) {
        const auto& key = key_holder_get_key(key_holder);
        emplace2(key_holder, it, inserted, hash(key), row);
    }

    template <typename KeyHolder>
    void ALWAYS_INLINE emplace2(KeyHolder&& key_holder, LookupResult& it, bool& inserted,
                               size_t hash_value, uint32_t row) {
        const auto& key = key_holder_get_key(key_holder);
        if (!emplace_if_zero(key, it, inserted, hash_value))
            emplace_non_zero(key_holder, it, inserted, hash_value, row);
    }

    template <typename KeyHolder>
    void ALWAYS_INLINE emplace2(KeyHolder&& key_holder, LookupResult& it, size_t hash_value,
                               bool& inserted, uint32_t row) {
        emplace2(key_holder, it, inserted, hash_value, row);
    }
...
//        }
    ...
    size_t get_buffer_size_in_bytes() const { return grower.buf_size() * sizeof(uint32_t); }
...
//        fprintf(stderr, "resize: %ld, %ld\n", for_num_elems, for_buf_size);
...
//        fprintf(stderr, "==> start resize hash table %lu ns, get_buffer_size_in_bytes(): %ld, new_grower.buf_size(): %ld, sizeof(Cell): %ld\n",
//                (endT.tv_sec - startT.tv_sec) * 1000000000 + (endT.tv_nsec - startT.tv_nsec), get_buffer_size_in_bytes(), new_grower.buf_size(), sizeof(Cell));
//        clock_gettime(CLOCK_MONOTONIC, &startT);
//        /// Expand the space.
//        buf = reinterpret_cast<Cell*>(Allocator::realloc(buf, get_buffer_size_in_bytes(),
//                                                         new_grower.buf_size() * sizeof(Cell)));
//        clock_gettime(CLOCK_MONOTONIC, &endT);
//        fprintf(stderr, "==> resize1 hash table %lu ns, get_buffer_size_in_bytes(): %ld, new_grower.buf_size(): %ld, sizeof(Cell): %ld\n",
//                (endT.tv_sec - startT.tv_sec) * 1000000000 + (endT.tv_nsec - startT.tv_nsec), get_buffer_size_in_bytes(), new_grower.buf_size(), sizeof(Cell));
//        grower = new_grower;
//
//        clock_gettime(CLOCK_MONOTONIC, &startT);
...
//            reinsert(buf[i], buf[i].get_hash(*this));
//        clock_gettime(CLOCK_MONOTONIC, &endT);
//        fprintf(stderr, "==> resize3 hash table %lu ns\n", (endT.tv_sec - startT.tv_sec) * 1000000000 + (endT.tv_nsec - startT.tv_nsec));
//
//#ifdef DBMS_HASH_MAP_DEBUG_RESIZES
//        watch.stop();
