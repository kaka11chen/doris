...
public:
    ...
    template <typename Data>
    ALWAYS_INLINE EmplaceResult emplace_key2(Data& data, size_t hash_value, size_t row,
                                            Arena& pool) {
        auto key_holder = static_cast<Derived&>(*this).get_key_holder(row, pool);
        return emplaceImpl2(key_holder, hash_value, data, row);
    }
...
protected:
    ...
    template <typename Data, typename KeyHolder>
    ALWAYS_INLINE EmplaceResult emplaceImpl2(KeyHolder& key_holder, size_t hash_value, Data& data, uint32_t row) {
        if constexpr (Cache::consecutive_keys_optimization) {
            if (cache.found && cache.check(key_holder_get_key(key_holder))) {
                if constexpr (has_mapped)
                    return EmplaceResult(cache.value.second, cache.value.second, false);
        ...
        typename Data::LookupResult it;
        bool inserted = false;
        data.emplace2(key_holder, it, hash_value, inserted, row);
