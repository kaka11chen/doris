...
Status IcebergInsertPartitionFunction::prepare(RuntimeState* state,
...
Status IcebergInsertPartitionFunction::get_partitions(
...
Status IcebergInsertPartitionFunction::clone(
...
IcebergDeletePartitionFunction::IcebergDeletePartitionFunction(
        HashValType partition_count, ShuffleHashMethod hash_method,
...
Status IcebergDeletePartitionFunction::prepare(RuntimeState* state,
                                               const RowDescriptor& row_desc) {
...
Status IcebergDeletePartitionFunction::get_partitions(
        RuntimeState* /*state*/, Block* block, size_t partition_count,
        std::vector<HashValType>& partitions) const {
    if (partition_count == 0) {
        return Status::InternalError("Partition count is zero");
...
Status IcebergDeletePartitionFunction::clone(
        RuntimeState* state, std::unique_ptr<PartitionFunction>& function) const {
    auto* new_function =
...
Status IcebergDeletePartitionFunction::_compute_hashes(
        Block* block, std::vector<HashValType>& partitions) const {
    const size_t rows = block->rows();
    if (rows == 0) {
        partitions.clear();
...
Status IcebergDeletePartitionFunction::_get_delete_hash_column(
        const ColumnWithTypeAndName& column, ColumnPtr* out_column,
