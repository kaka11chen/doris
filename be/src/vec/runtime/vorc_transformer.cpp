...
Status VOrcTransformer::open() {
    if (_schema_str != nullptr) {
        ...
        if (_iceberg_schema != nullptr) {
            fprintf(stderr, "_iceberg_schema->root_struct().fields().size(): %ld\n",
                    _iceberg_schema->root_struct().fields().size());
            nested_fields = &_iceberg_schema->root_struct().fields();
        }
        for (int i = 0; i < _output_vexpr_ctxs.size(); i++) {
            VExprSPtr column_expr = _output_vexpr_ctxs[i]->root();
            try {
                fprintf(stderr, "nested_fields is nullptr: %d\n", nested_fields == nullptr);
    ...
    _writer->addUserMetadata("CreatedBy", version::doris_build_version());
    return Status::OK();
}
...
std::unique_ptr<orc::Type> VOrcTransformer::_build_orc_type(
    ...
    fprintf(stderr, "type_descriptor: %s\n", type_descriptor.debug_string().c_str());
    ...
    if (nested_field != nullptr) {
        fprintf(stderr, "nested_field->field_id(): %d\n", nested_field->field_id());
        ...
        fprintf(stderr, "nested_field is null\n");
    }
    return type;
}
