...
Status VIcebergDeleteSink::close(Status close_status) {
    ...
    if (!close_status.ok()) {
        LOG(WARNING) << fmt::format("VIcebergDeleteSink close with error: {}", close_status.to_string());
        return close_status;
    }
    ...
    LOG(INFO) << fmt::format("VIcebergDeleteSink closed: rows={}, delete_files={}",
                            _row_count, _delete_file_count);
        ...
        if (!inserted) {
            if (iter->second.partition_spec_id != partition_spec_id ||
                ...
                LOG(WARNING) << fmt::format(
                        ...
                        iter->second.partition_data_json, partition_spec_id, partition_data_json);
            }
        }
    ...
    for (const auto& [data_file_path, deletion] : file_deletions) {
        ...
        for (auto it = deletion.rows_to_delete.begin(); it != deletion.rows_to_delete.end(); ++it) {
            ...
            if (positions.size() >= kBatchSize) {
                Block delete_block;
                RETURN_IF_ERROR(_build_position_delete_block(data_file_path, positions,
                                                             delete_block));
...
Status VIcebergDeleteSink::_init_position_delete_output_exprs() {
    ...
    TExprNode file_path_node = create_texpr_node_from(&empty_string, PrimitiveType::TYPE_STRING, 0,
                                                      0);
...
Status VIcebergDeleteSink::_build_position_delete_block(const std::string& file_path,
    ...
    // Build block
    output_block.insert(ColumnWithTypeAndName(
            std::move(file_path_col), std::make_shared<DataTypeString>(), "file_path"));
    output_block.insert(ColumnWithTypeAndName(
            std::move(pos_col), std::make_shared<DataTypeInt64>(), "pos"));
...
std::string VIcebergDeleteSink::_generate_delete_file_path(const std::string& referenced_data_file) {
