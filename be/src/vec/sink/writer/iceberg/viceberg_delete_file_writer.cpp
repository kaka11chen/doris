...
VIcebergDeleteFileWriter::VIcebergDeleteFileWriter(
        TFileContent::type delete_type,
...
Status VIcebergDeleteFileWriter::open(
    ...
    case TFileFormatType::FORMAT_ORC: {
        _file_format_transformer.reset(
                new VOrcTransformer(state, _file_writer.get(), output_exprs, "",
                                   column_names, false, _compress_type, nullptr));
        return _file_format_transformer->open();
    }
    default:
        return Status::InternalError("Unsupported file format type {}",
                                    to_string(_file_format));
    }
...
std::unique_ptr<VIcebergDeleteFileWriter> VIcebergDeleteFileWriterFactory::create_writer(
    ...
    return std::make_unique<VIcebergDeleteFileWriter>(
            delete_type, output_path, file_format, compress_type);
}
