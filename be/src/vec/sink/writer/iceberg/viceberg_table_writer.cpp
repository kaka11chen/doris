...
#include "vec/sink/writer/iceberg/sorting_file_writer.h"
...
Status VIcebergTableWriter::init_properties(ObjectPool* pool, const RowDescriptor& row_desc) {
    _pool = pool;
    _row_desc = &row_desc;
    return Status::OK();
}
...
//    fprintf(stderr, "print_use_count777: %s\n", block.print_use_count().c_str());
//    SCOPED_RAW_TIMER(&_send_data_ns);
//    std::unordered_map<std::shared_ptr<IPartitionWriter>, IColumn::Filter> writer_positions;
//    _row_count += block.rows();
//    if (_iceberg_partition_columns.empty()) {
//        Block transformed_block;
//        std::shared_ptr<IPartitionWriter> writer;
//        {
...
//                try {
//                    writer = _create_partition_writer(block, -1, transformed_block);
//                } catch (doris::Exception& e) {
//                    return e.to_status();
//                }
//                _partitions_to_writers.insert({"", writer});
//                RETURN_IF_ERROR(writer->open(_state, _profile, _row_desc, _pool));
//            } else {
...
//                    try {
//                        writer = _create_partition_writer(block, -1, transformed_block, &file_name,
//                                                          file_name_index + 1);
//                    } catch (doris::Exception& e) {
//                        return e.to_status();
//                    }
...
//                        std::shared_ptr<IPartitionWriter>& writer_ptr) -> Status {
//                try {
//                    auto writer =
//                            _create_partition_writer(block, position, transformed_block, file_name, file_name_index);
//                    RETURN_IF_ERROR(writer->open(_state, _profile, _row_desc, _pool));
...
Status VIcebergTableWriter::write(vectorized::Block& block) {
    fprintf(stderr, "VIcebergTableWriter::write\n");
    SCOPED_RAW_TIMER(&_send_data_ns);
    if (block.rows() == 0) {
        return Status::OK();
    }
    ...
    fprintf(stderr, "output_block print_use_count: %s\n", output_block.print_use_count().c_str());
    ...
    if (_iceberg_partition_columns.empty()) {
        std::shared_ptr<IPartitionWriter> writer;
        {
            ...
            if (writer_iter == _partitions_to_writers.end()) {
                try {
                    writer = _create_partition_writer(output_block, -1, nullptr);
                } catch (doris::Exception& e) {
                    return e.to_status();
                }
                _partitions_to_writers.insert({"", writer});
                ...
                if (writer_iter->second->written_len() > config::iceberg_sink_max_file_size) {
                    ...
                    _partitions_to_writers.erase(writer_iter);
                    try {
                        writer = _create_partition_writer(output_block, -1, nullptr, &file_name,
                                                          file_name_index + 1);
                    } catch (doris::Exception& e) {
                        return e.to_status();
                    }
        ...
        for (int i = 0; i < output_block.rows(); ++i) {
            ...
            auto create_and_open_writer =
                    [&](const std::string& partition_name, int position,
                        ...
                        std::shared_ptr<IPartitionWriter>& writer_ptr) -> Status {
                try {
                    auto writer = _create_partition_writer(output_block, position, &transformed_block, file_name,
                                                           file_name_index);
                    RETURN_IF_ERROR(writer->open(_state, _profile, _row_desc, _pool));
    ...
    SCOPED_RAW_TIMER(&_partition_writers_write_ns);
    output_block.erase(_non_write_columns_indices);
    fprintf(stderr, "writer_positions.size(): %ld\n", writer_positions.size());
    for (auto it = writer_positions.begin(); it != writer_positions.end(); ++it) {
        Block filtered_block;
        RETURN_IF_ERROR(_filter_block(output_block, &it->second, &filtered_block));
        fprintf(stderr, "it->first->write(filtered_block)\n");
...
std::shared_ptr<IPartitionWriter> VIcebergTableWriter::_create_partition_writer(
        vectorized::Block& block, int position, Block *transformed_block, const std::string* file_name, int file_name_index) {
    auto& iceberg_table_sink = _t_sink.iceberg_table_sink;
    std::optional<PartitionData> partition_data;
    partition_data = _get_partition_data(transformed_block, position);
    ...
    fprintf(stderr, "original_write_path: %s\n", original_write_path.c_str());
    fprintf(stderr, "target_path: %s\n", target_path.c_str());

    IPartitionWriter::WriteInfo write_info = {
...
//    if (sorting) {
//        auto iceberg_partition_writer = std::make_shared<IPartitionWriter>(
//            _t_sink, std::move(partition_values), _vec_output_expr_ctxs, _write_output_vexpr_ctxs,
//            _non_write_columns_indices, *_schema, &_t_sink.iceberg_table_sink.schema_json,
//            std::move(write_info), (file_name == nullptr) ? _compute_file_name() : *file_name,
//            file_name_index, iceberg_table_sink.file_format, iceberg_table_sink.compression_type,
...
//                iceberg_table_sink.hadoop_config);
//    }
    auto iceberg_partition_writer = std::make_shared<SortingFileWriter<VIcebergPartitionWriter>>(
...
std::optional<PartitionData> VIcebergTableWriter::_get_partition_data(
    ...
    for (auto& iceberg_partition_column : _iceberg_partition_columns) {
        ...
        fprintf(stderr, "transformed_block.columns(): %ld\n", transformed_block->columns());
        const TypeDescriptor& result_type =
                iceberg_partition_column.partition_column_transform().get_result_type();
        auto value = _get_iceberg_partition_value(result_type, partition_column, position);
        values.emplace_back(value);
        ++column_idx;
        fprintf(stderr, "partition_column.column->use_count(): %d\n", partition_column.column->use_count());
    }
...
std::any VIcebergTableWriter::_get_iceberg_partition_value(
        ...
        int position) {
    fprintf(stderr, "_get_iceberg_partition_value: position: %d\n", position);
    ColumnPtr column;
    if (auto* nullable_column = check_and_get_column<ColumnNullable>(*partition_column.column)) {
        ...
        fprintf(stderr, "_get_iceberg_partition_value: nullable_column\n");
        column = nullable_column->get_nested_column_ptr();
    } else {
        fprintf(stderr, "_get_iceberg_partition_value: not nullable_column\n");
