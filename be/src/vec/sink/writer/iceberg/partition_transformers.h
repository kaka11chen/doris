...
public:
    ...
    //        auto year_month = std::chrono::year_month_day(std::chrono::sys_days(EPOCH + std::chrono::months(month_ordinal)));
    //        int year = year_month.year();
    //        int month = static_cast<int>(year_month.month());
    //        return std::to_string(year) + "-" + (month < 10 ? "0" : "") + std::to_string(month);
    //    }
    //
    //    static std::string human_day(int day_ordinal) {
    //        auto day = std::chrono::sys_days(EPOCH + std::chrono::days(day_ordinal));
    //        auto year_month_day = std::chrono::year_month_day(day);
    //        int year = year_month_day.year();
    //        int month = static_cast<int>(year_month_day.month());
    //        int day_of_month = year_month_day.day();
    //        return std::to_string(year) + "-" + (month < 10 ? "0" : "") + std::to_string(month) + "-" + (day_of_month < 10 ? "0" : "") + std::to_string(day_of_month);
    //    }
    //
    //    static std::string human_time(long long micros_from_midnight) {
    //        auto time = std::chrono::seconds(micros_from_midnight / 1000000);
    //        auto seconds = std::chrono::duration_cast<std::chrono::seconds>(time).count();
    //        auto minutes = (seconds / 60) % 60;
    //        auto hours = (seconds / 3600) % 24;
    //        auto micros = micros_from_midnight % 1000000;
    //        std::ostringstream oss;
    //        oss << std::setw(2) << std::setfill('0') << hours << ":" << std::setw(2) << std::setfill('0') << minutes << ":" << std::setw(2) << std::setfill('0') << seconds << "." << std::setw(6) << std::setfill('0') << micros;
    //        return oss.str();
    //    }
    //
    //    static std::string human_timestamp_with_zone(long long timestamp_micros) {
    //        auto timestamp = EPOCH + std::chrono::microseconds(timestamp_micros);
    ...
    //    }
    //
    //    static std::string human_timestamp_without_zone(long long timestamp_micros) {
    //        auto timestamp = EPOCH + std::chrono::microseconds(timestamp_micros);
    //        auto time = std::chrono::system_clock::to_time_t(timestamp);
    //        std::ostringstream oss;
    ...
    //        auto hour = EPOCH + std::chrono::hours(hour_ordinal);
    //        auto year_month_day_hour = std::chrono::year_month_day(std::chrono::sys_days(hour));
    //        int year = year_month_day_hour.year();
    //        int month = static_cast<int>(year_month_day_hour.month());
    //        int day_of_month = year_month_day_hour.day();
    //        int hour_of_day = hour.time_since_epoch().count() % (24 * 3600 * 1000000) / (3600 * 1000000);
    //        return std::to_string(year) + "-" + (month < 10 ? "0" : "") + std::to_string(month) + "-" + (day_of_month < 10 ? "0" : "") + std::to_string(day_of_month) + "-" + (hour_of_day < 10 ? "0" : "") + std::to_string(hour_of_day);
    //    }

    //    static std::string base64encode(const std::vector<char>& buffer) {
    //        std::string encoded;
    //        const char* bytes = buffer.data();
    //        size_t size = buffer.size();
    //        for (size_t i = 0; i < size; i += 3) {
    //            uint32_t packed = 0;
    //            packed |= (static_cast<uint8_t>(bytes[i]) << 16);
    //            if (i + 1 < size) {
    //                packed |= (static_cast<uint8_t>(bytes[i + 1]) << 8);
    //            }
    //            if (i + 2 < size) {
    //                packed |= static_cast<uint8_t>(bytes[i + 2]);
    //            }
    //            for (int j = 0; j < 4; ++j) {
    //                if (i + j * 6 / 8 - 1 < size) {
    //                    encoded.push_back("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"[(packed >> (6 * (3 - j))) & 0x3F]);
    //                } else {
    //                    encoded.push_back('=');
    //                }
    //            }
